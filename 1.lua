...
local TestTab2 = Page:CreateSection({
    Name = "Tool",
    Side = "Left" -- ตำแหน่ง Left/Right
})

local Mic = Page:CreateSection({
    Name = "Other",
    Side = "Right" -- ตำแหน่ง Left/Right
})

local Mics = Page2:CreateSection({
    Name = "Role (Lobby)",
    Side = "Left" -- ตำแหน่ง Left/Right
})
...
TestTab:AddToggle({
    Name = "Auto Heal Other",
	Value = _G.Get,
    Callback = function(value)
        _G.Get = value
        while _G.Get do wait()
        local remoteEvents = game:GetService("ReplicatedStorage"):WaitForChild("RemoteEvents")
local healPlayerEvent = remoteEvents:WaitForChild("HealPlayer")
...
for _, player in ipairs(players) do
    ...
    healPlayerEvent:FireServer(unpack(args))
end
        end
    end
})
...
TestTab:AddToggle({
    ...
    Callback = function(value)
        ...
        while _G.Get do wait()
local remoteEvents = game:GetService("ReplicatedStorage"):WaitForChild("RemoteEvents")
local toxicDrownEvent = remoteEvents:WaitForChild("ToxicDrown")
local players = game:GetService("Players"):GetPlayers()
...
for _, player in ipairs(players) do
    if player ~= localPlayer then
        ...
        toxicDrownEvent:FireServer(unpack(args))
    end
end
        end
    end
...
for _, player in ipairs(players) do
    if player ~= localPlayer and calculateDistance(player) <= maxDistance then
        local args = {
            [1] = 1,
            [2] = player
        }
...
TestTab:AddToggle({
    Name = "Hit Bad Guys Tool Need!",
	Value = _G.Get,
    Callback = function(value)
        _G.Get = value
        while _G.Get do wait()
        local BadGuysFolder = workspace:WaitForChild("BadGuys")
local RemoteEvents = game:GetService("ReplicatedStorage"):WaitForChild("RemoteEvents")
local HitBadGuyEvent = RemoteEvents:WaitForChild("HitBadguy")

local args = {
    [2] = 8
}

for _, badGuy in ipairs(BadGuysFolder:GetChildren()) do
    args[1] = badGuy
...
local Money3 = getNil("Money3", "Part")
local Money2 = getNil("Money2", "Part")
local Money = getNil("Money", "Part")

if Money3 or Money2 or Money then
    local RemoteEvents = game:GetService("ReplicatedStorage"):WaitForChild("RemoteEvents")
    local FoundMoneyEvent = RemoteEvents:WaitForChild("FoundMoney")
    ...
    FoundMoneyEvent:FireServer(unpack(args))
end
    end
})
...
Mic:AddButton({
    Name = "Esp Bad Guys",
    Callback = function()
    local BadGuysFolder = game.Workspace.BadGuys

for _, v in pairs(BadGuysFolder:GetChildren()) do
    if v:IsA("Model") and v:GetChildren() then
        local esp = Instance.new("BoxHandleAdornment")
        esp.Name = v.Name .. "_ESP"
        esp.Size = v.PrimaryPart.Size
        esp.Color3 = Color3.new(0, 1, 0)
        esp.Transparency = 0.4
        esp.Adornee = v.PrimaryPart
        esp.ZIndex = 10
        esp.AlwaysOnTop = true
        esp.Parent = v.PrimaryPart
    end
end
    end
})
...
Mic:AddButton({
    Name = "Teleport to random Players",
    Callback = function()
...
local function teleportToRandomPlayer()
    ...
    if #randomPlayer > 0 then
        local randomIndex = math.random(1, #randomPlayer)
        local targetPlayer = randomPlayer[randomIndex]
...
Mics:AddButton({
    Name = "Hyper",
    Callback = function()
game:GetService("ReplicatedStorage").RemoteEvents.OutsideRole:FireServer("Lollipop",true)
    end
})
...
Mics:AddButton({
    Name = "Teddy Blox Pin",
    Callback = function()
game:GetService("ReplicatedStorage").RemoteEvents.MakeRole:FireServer("TeddyBloxpin",true,true)   
   end
})
...
Mics:AddButton({
    Name = "Chip",
    Callback = function()
game:GetService("ReplicatedStorage").RemoteEvents.MakeRole:FireServer("Chips",true,true)
   end
})
...
Mics:AddButton({
    Name = "Bat",
    Callback = function()
game:GetService("ReplicatedStorage").RemoteEvents.MakeRole:FireServer("Bat",false,true)
   end
})
...
Mics:AddButton({
    Name = "MedKit",
    Callback = function()
game:GetService("ReplicatedStorage").RemoteEvents.MakeRole:FireServer("MedKit",false,true)
   end
})
...
return library, library_flags, library.subsw
